...
pub async fn get_proofs_with_authorities(
    client: &RpcClient,
    authorities: &[Pubkey],
) -> HashMap<Pubkey, Proof> {
    let mut proof_addresses = Vec::new();
    for author in authorities.into_iter() {
        proof_addresses.push(proof_pubkey(author.clone()))
    }
    get_proofs(client, &proof_addresses.as_slice()).await
}
...
pub async fn get_updated_proofs_with_authorities(
    client: &RpcClient,
    authorities: &[Pubkey],
) -> HashMap<Pubkey, Proof> {
    get_proofs_with_authorities(client, authorities).await
}
...
pub async fn get_proofs(client: &RpcClient, addresses: &[Pubkey]) -> HashMap<Pubkey, Proof> {
    let data = client
        .get_multiple_accounts(&addresses)
        .await
        .expect("Failed to get proofs account");
    let mut hash = HashMap::new();
    for (i, d) in data.iter().enumerate() {
        match d {
            Some(acc) => match Proof::try_from_bytes(&acc.data) {
                Ok(proof) => {
                    hash.insert(addresses[i], *proof);
                }
                Err(e) => {
                    eprintln!("{}", e);
                }
            ...
            None => {
                println!("No proof from address {}", addresses[i])
            }
        }
    }
    return hash;
